...
$((function() {
	...
	var a = $(".datatables-products"),
		s = baseUrl + "app/ecommerce/product/add",
		i = {
			1: {
				title: "Scheduled",
		...
		a.DataTable({
			...
			columnDefs: [{
				...
				render: function(t, e, n, a) {
					var s = n.product_name,
						...
						o = n.product_brand,
						r = n.image;
					if (r) var c = '<img src="' + assetsPath + "img/ecommerce-images/" + r + '" alt="Product-' + i + '" class="rounded-2">';
					else {
						var l = ["success", "danger", "warning", "info", "dark", "primary", "secondary"][Math.floor(6 * Math.random())],
							d = (s = n.product_brand).match(/\b\w/g) || [];
						c = '<span class="avatar-initial rounded-2 bg-label-' + l + '">' + (d = ((d.shift() || "") + (d.pop() || "")).toUpperCase()) + "</span>"
					}
					return '<div class="d-flex justify-content-start align-items-center product-name"><div class="avatar-wrapper me-3"><div class="avatar rounded-2 bg-label-secondary">' + c + '</div></div><div class="d-flex flex-column"><span class="text-nowrap text-heading fw-medium">' + s + '</span><small class="text-truncate d-none d-sm-block">' + o + "</small></div></div>"
				}
			}, {
				targets: 3,
				responsivePriority: 5,
				render: function(t, e, n, a) {
					var s = o[n.category].title;
				...
				render: function(t, e, n, a) {
					...
					} [r[s].title] + '<span class="d-none">' + r[s].title + "</span></span>"
				}
			}, {
				targets: 5,
				render: function(t, e, n, a) {
				...
				render: function(t, e, n, a) {
					var s = n.status;
					return '<span class="badge rounded-pill ' + i[s].class + '" text-capitalized>' + i[s].title + "</span>"
				}
			}, {
				targets: -1,
				title: "Actions",
			...
			initComplete: function() {
				this.api().columns(-2).every((function() {
					var t = this,
						e = $('<select id="ProductStatus" class="form-select text-capitalize"><option value="">Status</option></select>').appendTo(".product_status").on("change", (function() {
							var e = $.fn.dataTable.util.escapeRegex($(this).val());
							t.search(e ? "^" + e + "$" : "", !0, !1).draw()
						}));
					t.data().unique().sort().each((function(t, n) {
						e.append('<option value="' + i[t].title + '">' + i[t].title + "</option>")
					}))
				})), this.api().columns(3).every((function() {
					var t = this,
						e = $('<select id="ProductCategory" class="form-select text-capitalize"><option value="">Category</option></select>').appendTo(".product_category").on("change", (function() {
							var e = $.fn.dataTable.util.escapeRegex($(this).val());
							t.search(e ? "^" + e + "$" : "", !0, !1).draw()
						}));
					t.data().unique().sort().each((function(t, n) {
						e.append('<option value="' + o[t].title + '">' + o[t].title + "</option>")
					}))
				})), this.api().columns(4).every((function() {
					var t = this,
						e = $('<select id="ProductStock" class="form-select text-capitalize"><option value=""> Stock </option></select>').appendTo(".product_stock").on("change", (function() {
							var e = $.fn.dataTable.util.escapeRegex($(this).val());
							t.search(e ? "^" + e + "$" : "", !0, !1).draw()
						}));
					t.data().unique().sort().each((function(t, n) {
						e.append('<option value="' + r[t].title + '">' + c[t].title + "</option>")
					}))
				}))
			}
		});
